...
## What each top-level file does (concise) 📁
- `init.lua` — Bootstraps `lazy.nvim`, loads `vim-options` and the `plugins` directory.
- `lazy-lock.json` — Lockfile generated by `lazy.nvim` (pins plugin versions).
- `lua/vim-options.lua` — Editor options (tab size, leader, clipboard, line numbers, etc.).
- `lua/plugins.lua` — Central module (currently empty); plugin specs are provided by modules in `lua/plugins/`.
- `lua/plugins/*.lua` — Each file exports one or more plugin specs for `lazy.nvim`. See the breakdown below.
...
- generate a short `CONTRIBUTING.md` for adding plugins, or
- add a `settings` section documenting your personal keybindings in more detail.
